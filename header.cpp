#include <iostream>
#include <fstream>
#include <sstream>
#include <cstring>
#include "header.h"

#include "mainwindow.h"
#include "./ui_mainwindow.h"
#include <QApplication>
using namespace std;

TrieNode::TrieNode(char c)
{
    data = c;
    isEnd = false;
    children = new TrieNode*[26]();
}

TrieNode::~TrieNode()
{
    for (int i = 0; i < 26; ++i)
    {
        delete children[i];
    }
    delete[] children;
}

// Trie implementation
Trie::Trie()
{
    root = new TrieNode('\0');
}
Trie::~Trie()
{
    delete root;
}
void Trie::insertWord(TrieNode* node, const string& word)
{
    for (char c : word)
    {
        int index = c - 'a';
        if (!node->children[index])
        {
            node->children[index] = new TrieNode(c);
        }
        node = node->children[index];
    }
    node->isEnd = true;
}
TrieNode* Trie::findNode(TrieNode* node, const string& prefix)
{
    for (char c : prefix)
    {
        int index = c - 'a';
        if (!node->children[index])
        {
            return nullptr; // Prefix not found
        }
        node = node->children[index];
    }
    return node;
}
void Trie::getSuggestions(TrieNode* node, const string& prefix, Ui::MainWindow *ui)
{

    if (node == nullptr)
    {
        ui->OutputBox->append("No Sugestion Found!\n");
        return;
    }
    if (node->isEnd)
    {
        cout << prefix << endl;
        ui->OutputBox->append(QString::fromStdString("> " + prefix));
        //ui->OutputBox->append("\n");

    }
    for (int i = 0; i < 26; ++i)
    {
        if (node->children[i])
        {
            char nextChar = 'a' + i;
            getSuggestions(node->children[i], prefix + nextChar, ui);
        }
    }
}
void Trie::insert(const string& word)
{
    insertWord(root, word);
}
void Trie::getSuggestions(const string& prefix, Ui::MainWindow *ui)
{
    TrieNode* node = findNode(root, prefix);
    getSuggestions(node, prefix, ui);
}

// AVLNode implementation
AVLNode::AVLNode(string word, string wordMeaning)
{
    key = word;
    meaning = wordMeaning;
    left = NULL;
    right = NULL;
    height = 1;
}

// AVLTree implementation
AVLTree::AVLTree()
{
    root = NULL;
}
int AVLTree::getHeight(AVLNode* node)
{
    if (node == NULL)
    {
        return 0;
    }
    else
    {
        return node->height;
    }
}
int AVLTree::getBalanceFactor(AVLNode* node)
{
    if (node == NULL)
    {
        return 0;
    }
    else
    {
        return getHeight(node->left) - getHeight(node->right);
    }
}
AVLNode* AVLTree::rotateRight(AVLNode* root)
{
    AVLNode* temp = root->left;
    AVLNode* T2 = temp->right;
    temp->right = root;
    root->left = T2;
    root->height = max(getHeight(root->left), getHeight(root->right)) + 1;
    temp->height = max(getHeight(temp->left), getHeight(temp->right)) + 1;
    return temp;
}
AVLNode* AVLTree::rotateLeft(AVLNode* root)
{
    AVLNode* temp = root->right;
    AVLNode* T2 = temp->left;
    temp->left = root;
    root->right = T2;
    root->height = max(getHeight(root->left), getHeight(root->right)) + 1;
    temp->height = max(getHeight(temp->left), getHeight(temp->right)) + 1;
    return temp;
}
AVLNode* AVLTree::insert(AVLNode* node, string key, string meaning)
{
    if (node == NULL)
    {
        return new AVLNode(key, meaning);
    }
    if (key < node->key)
    {
        node->left = insert(node->left, key, meaning);
    }
    else if (key > node->key)
    {
        node->right = insert(node->right, key, meaning);
    }
    else
    {
        // Duplicate keys are not allowed
        return node;
    }
    node->height = 1 + max(getHeight(node->left), getHeight(node->right));
    int balance = getBalanceFactor(node);
    // Left Left Case
    if (balance > 1 && key < node->left->key)
    {
        return rotateRight(node);
    }
    // Right Right Case
    if (balance < -1 && key > node->right->key)
    {
        return rotateLeft(node);
    }
    // Left Right Case
    if (balance > 1 && key > node->left->key)
    {
        node->left = rotateLeft(node->left);
        return rotateRight(node);
    }
    // Right Left Case
    if (balance < -1 && key < node->right->key)
    {
        node->right = rotateRight(node->right);
        return rotateLeft(node);
    }
    return node;
}
AVLNode* AVLTree::search(AVLNode* node, const string& key)
{
    if (node == NULL || key == node->key)
    {
        return node;
    }
    if (key < node->key)
    {
        return search(node->left, key);
    }
    else
    {
        return search(node->right, key);
    }
}
AVLNode* AVLTree::findMinNode(AVLNode* node)
{
    while (node->left != NULL)
    {
        node = node->left;
    }
    return node;
}
AVLNode* AVLTree::deleteNode(AVLNode* root, string key)
{
    if (root == NULL)
    {
        return root;
    }
    if (key < root->key)
    {
        root->left = deleteNode(root->left, key);
    }
    else if (key > root->key)
    {
        root->right = deleteNode(root->right, key);
    }
    else {
        // Node with only one child or no child
        if (root->left == NULL)
        {
            AVLNode* temp = root->right;
            delete root;
            return temp;
        }
        else if (root->right == NULL)
        {
            AVLNode* temp = root->left;
            delete root;
            return temp;
        }
        // Node with two children, get the inorder successor (smallest in the right subtree)
        AVLNode* temp = findMinNode(root->right);
        // Copy the inorder successor's data to this node
        root->key = temp->key;
        root->meaning = temp->meaning;
        // Delete the inorder successor
        root->right = deleteNode(root->right, temp->key);
    }
    // Update height and balance factor
    root->height = 1 + max(getHeight(root->left), getHeight(root->right));
    int balance = getBalanceFactor(root);
    // Left Left Case
    if (balance > 1 && getBalanceFactor(root->left) >= 0)
    {
        return rotateRight(root);
    }
    // Left Right Case
    if (balance > 1 && getBalanceFactor(root->left) < 0)
    {
        root->left = rotateLeft(root->left);
        return rotateRight(root);
    }
    // Right Right Case
    if (balance < -1 && getBalanceFactor(root->right) <= 0)
    {
        return rotateLeft(root);
    }
    // Right Left Case
    if (balance < -1 && getBalanceFactor(root->right) > 0)
    {
        root->right = rotateRight(root->right);
        return rotateLeft(root);
    }
    return root;
}
void AVLTree::inOrderTraversalHelper(AVLNode* node, ostream& output)
{
    if (node != NULL)
    {
        inOrderTraversalHelper(node->left, output);
        output << node->key << " " << node->meaning << endl;
        inOrderTraversalHelper(node->right, output);
    }
}
void AVLTree::inOrderTraversal(AVLNode* node, ofstream& outputFile)
{
    inOrderTraversalHelper(node, outputFile);
}
void AVLTree::updateSuggestionsTrie(AVLNode* node)
{
    if (node != NULL)
    {
        updateSuggestionsTrie(node->left);
        suggestionsTrie.insert(node->key);
        updateSuggestionsTrie(node->right);
    }
}
void AVLTree::insert(string key, string meaning)
{
    root = insert(root, key, meaning);
    suggestionsTrie.insert(key); // Update suggestions trie
}
void AVLTree::loadFromFile(const string& filename)
{
    ifstream inputFile(filename);
    if (!inputFile.is_open())
    {
        cerr << "Error opening the file.\n";
        return;
    }
    cout << "Inserting words and meanings from file into AVL tree:\n";
    string word, meaning;
    // Read word and meaning from the file
    while (inputFile >> word)
    {
        // Read the rest of the line as the meaning
        getline(inputFile >> ws, meaning);
        insert(word, meaning);
    }
    cout << endl << "File Successfully Loaded!!!\n";
    inputFile.close();
}
void AVLTree::addWordToFile(const string& filename, string key, string meaning)
{
    ofstream outputFile(filename, ios::app);
    if (!outputFile.is_open())
    {
        cerr << "Error opening the file.\n";
        return;
    }
    // Append the word and its meaning to the file
    outputFile << key << " " << meaning << endl;
    outputFile.close();
    // After adding the word to the file, also update the AVL tree
    insert(key, meaning);
}
AVLNode* AVLTree::searchWord(const string& key)
{
    return search(root, key);
}
void AVLTree::updateWordMeaning(const string& key, const string& newMeaning)
{
    AVLNode* node = searchWord(key);
    if (node != NULL)
    {
        node->meaning = newMeaning;
        cout << "Word updated successfully!\n";
    }
    else
    {
        cout << "Word not found.\n";
    }
}
void AVLTree::deleteWord(const string& key, const string& filename)
{
    root = deleteNode(root, key);
    // Update the file after deletion
    ofstream outputFile(filename);
    if (!outputFile.is_open())
    {
        cerr << "Error opening the file.\n";
        return;
    }
    // Update the file with the remaining words in the AVL tree
    inOrderTraversal(root, outputFile);
    outputFile.close();
    // Update suggestions trie after deletion
    suggestionsTrie = Trie();
    updateSuggestionsTrie(root);
    cout << "Word deleted successfully!\n";
}
void AVLTree::displayInOrder(ostream& output)
{
    if (root == NULL)
    {
        output << "AVL tree is empty.\n";
        return;
    }
    inOrderTraversalHelper(root, output);
}
void AVLTree::displaySuggestions(const string& prefix, Ui::MainWindow *ui)
{
    cout << "\nSuggestions for prefix '" << prefix << "':\n";
    ui->OutputBox->setText("Suggestions are:\n");
    suggestionsTrie.getSuggestions(prefix, ui);
}
AVLTree avlTree;
